...
function Stars({ dark = false }: { dark?: boolean }) {
  return (
    <div className={`flex gap-1 ${dark ? "text-rose" : "text-orchid"}`} aria-hidden>
    ...
    <section className="bg-ink py-24 md:py-32">
      <div className="container-studio">
        <div className="mb-14 flex flex-col gap-10 lg:mb-20 lg:flex-row lg:items-end lg:justify-between">
          ...
          <Reveal delay={120}>
            <div className="flex gap-8 border border-white/20 bg-white/5 px-7 py-6">
              <div>
                <div className="flex items-baseline gap-1.5">
                  <span className="font-serif text-3xl text-white">{reviewStats.rating}</span>
              ...
              <div className="border-l border-white/20 pl-8">
                <div className="font-serif text-3xl text-white">{reviewStats.count}</div>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
          {testimonials.map((review, i) => (
            <Reveal key={review.name} delay={(i % 4) * 100}>
              <figure className="flex h-full flex-col gap-4 border border-border bg-white p-6 transition-shadow duration-500 hover:shadow-[0_20px_50px_-30px_rgba(0,0,0,0.4)]">
                <Stars />
